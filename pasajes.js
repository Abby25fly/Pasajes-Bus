...
function reserva () {
   ...
   while (true){
      ...
      if (option == 0) {
         ...
         if (nro > 0 && nro <= N) {
            var name =  prompt( "Nombre y Apellidos del pasajero:" )  ;
            var id = parseInt( prompt( "DNI del pasajero:"  ) );
            asientos[nro - 1] = {
               nombre : name,
               dni: id
            };
           document.write("Número de Asiento: " + nro + "<br/>" + "Nombre del pasajero: " + asientos [nro - 1].nombre + "<br/>" +
            "DNI del pasajero: " + asientos [nro - 1].dni + "<br/>");
         }
      }else if (option == 2) {
         str = "seleccione asiento: " + mostrar(asientos);
         var nro = parseInt( prompt( str )  );
